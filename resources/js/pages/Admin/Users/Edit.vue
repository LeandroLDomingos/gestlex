...
import type { BreadcrumbItem, User, Role, SharedData } from '@/types';
...
interface Props {
    user: User & { roles: Pick<Role, 'id' | 'name'>[] }; // Utilizador a ser editado, com seus papéis
    roles: Pick<Role, 'id' | 'name'>[]; // Lista de todos os papéis disponíveis
    userRoleIds: string[]; // IDs dos papéis atualmente atribuídos ao utilizador
    // canAssignRoles?: boolean; // Prop removida do controller
...
const form = useForm({
    ...
    roles: props.userRoleIds || [], // Inicializa com os IDs dos papéis do utilizador
});
...
const submitForm = () => {
    form.put(route('admin.users.update', props.user.id), {
        ...
        onSuccess: () => {
            ...
            form.password = ''; // Limpa campos de senha após sucesso
            form.password_confirmation = '';
        },
        onError: (formErrors) => {
            console.error("Erros ao atualizar utilizador:", formErrors);
             if (Object.keys(formErrors).length > 0) {
                 toast({ title: 'Erro de Validação', description: 'Por favor, corrija os erros no formulário.', variant: 'destructive' });
            } else {
                 toast({ title: 'Erro Inesperado', description: 'Não foi possível atualizar o utilizador.', variant: 'destructive' });
            }
        }
    });
};
                ...
                <Card class="shadow-xl">
                    <form @submit.prevent="submitForm">
                        <CardHeader>
                            ...
                            <CardDescription>Modifique as informações do utilizador abaixo.</CardDescription>
                        </CardHeader>
                        <CardContent class="space-y-6">
                            ...
                            <Separator />
                             <h3 class="text-md font-medium text-slate-800 dark:text-slate-100 pt-2">Alterar Senha (Opcional)</h3>
                            <div class="grid grid-cols-1 sm:grid-cols-2 gap-6">
                                <div>
                                    <Label for="password">Nova Senha</Label>
                                    <Input id="password" v-model="form.password" type="password" class="mt-1 block w-full" placeholder="Deixe em branco para não alterar" />
                            ...
                            <div>
                                ...
                                <div v-if="props.roles && props.roles.length > 0" class="space-y-3 max-h-60 overflow-y-auto p-1 rounded-md border">
                                    <div v-for="role in props.roles" :key="role.id" class="flex items-center space-x-3 p-2 hover:bg-slate-50 dark:hover:bg-slate-700/50 rounded">
                                        ...
                                        />
                                        <Label :for="`role-edit-${role.id}`" class="text-sm font-medium text-slate-700 dark:text-slate-200 cursor-pointer">
                                            {{ role.name }}
                                        </Label>
                                    </div>
                                </div>
